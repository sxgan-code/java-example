<script setup lang="ts">
const props = defineProps({
  fill: {type: String, default: '#FFFFFF', required: false},
  width: {type: String, default: '2rem', required: false},
  height: {type: String, default: '2rem', required: false},
})</script>
<template>
  <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24" height="24"
       version="1.1" viewBox="0 0 24 24">
    <defs>
      <path id="uxs-a"
            d="M4,4.5 C3.17157288,4.5 2.5,5.17157288 2.5,6 L2.5,18 C2.5,18.8284271 3.17157288,19.5 4,19.5 L20,19.5 C20.8284271,19.5 21.5,18.8284271 21.5,18 L21.5,6 C21.5,5.17157288 20.8284271,4.5 20,4.5 L4,4.5 Z M4,3 L20,3 C21.6568542,3 23,4.34314575 23,6 L23,18 C23,19.6568542 21.6568542,21 20,21 L4,21 C2.34314575,21 1,19.6568542 1,18 L1,6 C1,4.34314575 2.34314575,3 4,3 Z"/>
    </defs>
    <use fill="#000" fill-rule="nonzero" stroke="none" stroke-width="1" xlink:href="#uxs-a"/>
  </svg>
</template>
<style scoped lang="scss">
svg {
  width: v-bind(width);
  height: v-bind(height);
}
</style>